...
// TARGET: Yoda
//
// Edinburgh palette: muted greens, creams, earth tones.
// Heuristics: Green skin, large pointed ear shapes, small wise eyes.
// =============================================================================
const { createCanvas, loadImage } = require("canvas");

const name = "Yoda";

// Edinburgh for muted swamp greens + warm earth tones
const traits = {
...
const textPrompts = [
  "Yoda",
  "Yoda from Star Wars, green face with pointy ears",
  "small green alien with large ears",
  "Baby Yoda, Grogu",
];
...
function isGreen(h, s, b) {
  return h >= 60 && h <= 170 && s >= 10 && b >= 15 && b <= 80;
}

function isBrown(h, s, b) {
  return h >= 20 && h <= 55 && s >= 15 && b >= 15 && b <= 60;
}
...
async function scoreImage(imagePath) {
  ...
  // Score 2: Brown robes/earth tones (0-20)
  const brownRatio = brownPixels / totalPixels;
  let brownScore = 0;
  if (brownRatio >= 0.05 && brownRatio <= 0.4) {
    brownScore = 20;
  } else if (brownRatio >= 0.01 && brownRatio < 0.05) {
    brownScore = 20 * (brownRatio - 0.01) / 0.04;
  } else if (brownRatio > 0.4) {
    brownScore = 20 * Math.max(0, 1 - (brownRatio - 0.4) / 0.2);
  }

  // Score 3: Eye-like blobs (0-30)
  const minBlobSize = Math.floor(totalPixels * 0.001);
  ...
  for (let i = 0; i < upperBlobs.length; i++) {
    for (let j = i + 1; j < upperBlobs.length; j++) {
      ...
      const sizeScore = Math.min(avgSize / (totalPixels * 0.008), 1.0);
